  ...
  return (
    <div className={`min-h-screen ${experience.pageClass} ${experience.fontClass}`}>
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-20 pt-6 sm:px-6 lg:px-8">
        ...
        ) : experience.key === "housesdecors" ? (
          <section className={`overflow-hidden rounded-[2.2rem] ${experience.panelClass}`}>
            <div className="grid gap-0 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
              <div className="p-6 sm:p-8">
                <div className="inline-flex items-center gap-2 rounded-full border border-amber-300 bg-white/85 px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.24em] text-amber-900">
                  <Sparkles className="h-4 w-4" />
                  Profile detail
                </div>
                <h1 className="mt-5 text-4xl font-semibold tracking-[-0.05em] text-amber-950 sm:text-5xl">{brandName}</h1>
                {domain ? (
                  <div className={`mt-4 inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm ${experience.softPanelClass}`}>
                    <Globe className="h-4 w-4" />
                    {domain}
                  </div>
                ) : null}
                <p className={`mt-5 max-w-xl text-sm leading-8 ${experience.mutedClass}`}>
                  A cleaner identity-led layout helps profiles feel more credible and easier to scan before someone opens the linked website.
                </p>
                <div className="mt-7">{renderStats(experience)}</div>
              </div>

              <div className="grid gap-5 border-t border-amber-200/80 p-6 sm:p-8 xl:border-l xl:border-t-0">
                <div className="relative aspect-[16/10] overflow-hidden rounded-[2rem] bg-amber-100">
                  {coverUrl ? <ContentImage src={coverUrl} alt={`${brandName} cover`} fill className="object-cover" /> : null}
                </div>
                <div className="flex items-center gap-4">
                  <div className="relative h-24 w-24 overflow-hidden rounded-[1.7rem] border border-amber-200 bg-white shadow-sm">
                    {logoUrl ? <ContentImage src={logoUrl} alt={brandName} fill className="object-cover" /> : null}
                  </div>
                  <div className={`rounded-[1.5rem] p-4 ${experience.softPanelClass}`}>
                    <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-amber-900/60">Brand profile</p>
                    <p className="mt-2 text-sm leading-7 text-amber-950">
                      The card and content structure now mirror the more visual browsing style used on the image pages.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <section className="mt-10 grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          <div className={`rounded-[2rem] p-6 ${experience.panelClass}`}>
            <RichContent html={descriptionHtml} className="mx-auto max-w-none prose-p:my-4" />
          </div>
