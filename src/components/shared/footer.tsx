...
export function Footer() {
  ...
  if (recipe.footer === 'dense-footer') {
    return (
      <footer className="border-t border-[#0f241c] bg-[linear-gradient(180deg,#1b4332_0%,#143528_52%,#0f241c_100%)] text-white">
        <div className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr_1fr]">
            <div className="rounded-[2rem] border border-white/12 bg-[#0f241c]/55 p-7 shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]">
              ...
              {primaryTask ? (
                <Link href={primaryTask.route} className="mt-6 inline-flex items-center gap-2 rounded-full bg-white px-4 py-2.5 text-sm font-semibold text-[#1b4332] shadow-sm transition-colors hover:bg-emerald-50">
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:col-span-2 lg:grid-cols-3">
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-100/60">Explore</h3>
                <ul className="mt-4 space-y-3 text-sm text-emerald-50/90">
                  {footerLinks.platform.map((item: any) => (
                    <li key={item.name}><Link href={item.href} className="flex items-center gap-2 transition-colors hover:text-white">{item.icon ? <item.icon className="h-4 w-4 opacity-80" /> : null}{item.name}</Link></li>
                  ))}
                </ul>
              </div>
              <div>
                <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-emerald-100/60">Resources</h3>
                <ul className="mt-4 space-y-3 text-sm text-emerald-50/90">
                  {footerLinks.resources.map((item) => (
                    <li key={item.name}><Link href={item.href} className="transition-colors hover:text-white">{item.name}</Link></li>
                  ))}
                </ul>
              </div>
              <div>
          ...
          <div>
            <Link href="/" className="flex items-center gap-3">
              <div className="h-11 w-11 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1 shadow-sm">
                <img src="/favicon.png?v=20260418" alt={`${SITE_CONFIG.name} logo`} width="44" height="44" className="h-full w-full object-contain" />
              </div>
              <div>
