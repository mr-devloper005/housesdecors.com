...
export function TaskListClient({ task, initialPosts, category }: Props) {
  ...
  const merged = useMemo(() => {
    const getPostCategory = (post: SitePost) => {
      ...
      if (typeof (content as any).category === "string" && (content as any).category.trim()) {
        return normalizeCategory((content as any).category);
      }
      const firstTag = Array.isArray(post.tags) ? post.tags.find((tag) => typeof tag === "string" && tag.trim()) : "";
      return typeof firstTag === "string" && firstTag.trim() ? normalizeCategory(firstTag) : "";
    };
    ...
    if (normalizedCategory === "all") {
      return combined.filter((post) => {
        const value = getPostCategory(post);
        return !value || isValidCategory(value);
      });
    }

    return combined.filter((post) => getPostCategory(post) === normalizedCategory);
