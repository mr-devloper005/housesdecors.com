...
type TaskIntroEntry = (typeof taskIntroCopy)[TaskKey];
...
function renderHero(
  ...
  if (experience.key === "housesdecors") {
    const notesTitle = task === "profile" ? "Identity pages" : "Material panels";
    const descriptionCopy =
      task === "profile"
        ? "Profiles now lean more visual and trust-first so people can compare creators or brands faster."
        : experience.heroDescription;

    return (
      <section className="mb-10 grid gap-5 xl:grid-cols-[minmax(0,0.95fr)_minmax(250px,290px)_minmax(0,0.82fr)]">
        <div className={`rounded-[2rem] p-5 ${experience.panelClass}`}>
          <div>
            {intro ? (
              <>
                ...
                {intro.paragraphs.slice(0, 2).map((paragraph) => (
                  <p key={paragraph.slice(0, 30)} className={`mt-3 max-w-3xl text-sm leading-7 ${experience.mutedClass}`}>
                    {paragraph}
                  </p>
                ))}
                <div className="my-4 h-px bg-[#1b4332]/10" />
              </>
            ) : null}
            <p className={`text-xs font-semibold uppercase tracking-[0.3em] ${experience.mutedClass}`}>{experience.heroEyebrow}</p>
            <h1 className="mt-3 text-4xl font-semibold tracking-[-0.05em] text-amber-950 xl:text-[2.6rem]">{description}</h1>
            <p className={`mt-3 max-w-xl text-sm leading-6 ${experience.mutedClass}`}>{descriptionCopy}</p>
          </div>
        </div>
        <div className={`rounded-[2rem] p-5 ${experience.panelClass}`}>
          <div className={`rounded-[1.5rem] p-5 ${experience.softPanelClass}`}>
            ...
            <p className="mt-2 text-xl font-semibold text-amber-950">{notesTitle}</p>
          </div>
        ...
        <div className={`rounded-[2rem] p-6 ${experience.panelClass}`}>
          ...
          <div className="mt-4 flex flex-col gap-3 text-sm">
            <Link href={route} className={`inline-flex items-center gap-2 ${experience.mutedClass}`}>
...
export async function TaskListPage({ task, category }: { task: TaskKey; category?: string }) {
  if (TASK_LIST_PAGE_OVERRIDE_ENABLED) {
    const override = await TaskListPageOverride({ task, category });
  ...
  return (
    <div className={`min-h-screen ${experience.pageClass} ${experience.fontClass}`}>
      ...
      <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {intro && experience.key !== "housesdecors" ? (
