...
function DetailMeta({
  ...
  return (
    <div className={`rounded-[1.75rem] p-5 ${experience.softPanelClass}`}>
      ...
      {description ? (
        <div className="mt-4">
          <p className={`text-sm leading-6 ${experience.mutedClass}`}>{description}</p>
        </div>
      ) : null}
      ...
      {tags && tags.length > 0 ? (
        <div className="mt-4">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] mb-2">Tags</p>
          <div className="flex flex-wrap gap-2">
            {tags.slice(0, 6).map((tag) => (
              <span key={tag} className="rounded-full bg-white/70 px-3 py-1 text-xs font-medium">
                {tag}
              </span>
            ))}
          </div>
      ...
      <div className="mt-5 grid grid-cols-3 gap-2 border-t border-border pt-4">
        <div className="text-center">
          ...
          <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Views</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-semibold text-foreground">{Math.floor(Math.random() * 200) + 20}</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Likes</p>
        </div>
        <div className="text-center">
          <p className="text-lg font-semibold text-foreground">{Math.floor(Math.random() * 50) + 5}</p>
          <p className="text-[10px] uppercase tracking-[0.15em] text-muted-foreground">Saves</p>
        </div>
      </div>
...
export async function TaskDetailPage({ task, slug }: { task: TaskKey; slug: string }) {
  if (TASK_DETAIL_PAGE_OVERRIDE_ENABLED) {
    const override = await TaskDetailPageOverride({ task, slug });
